...
## Categories
1. De novo assembly-related
1. Annotation-related
1. Genome-wide statistics
1. Illumina data parsing
...
1. Scaffold ID
1. Position in reference FASTA
1. 1 or 0 indicating whether or not the sample FASTA is polymorphic or divergent relative to the reference FASTA
1. 1 or 0 indicating whether or not either FASTA has an N at this position
...
1. Scaffold ID
1. Position
1. Dxy between populations 1 and 2
1. Whether this site should be omitted
...
D_{x, y} = \frac{n}{n-1} \sum_{i,j \in {A,C,G,T}} 2*p_{x,i}*p_{y,j}
And Pi is calculated as:
Pi_{x} = \frac{n}{n-1} \sum_{i < j} 2*p_{x,i}*p_{x,j}
...
1. Index sequence
1. Count of index reads matching the sequence in column 1
