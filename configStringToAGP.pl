...
my $gap_size = 500;
my $dispversion = 0;
GetOptions('input_file|i=s' => \$input_path, 'config_string|c=s' => \$config_string_file, 'agp_file|a=s' => \$agp_file, 'unscaffolded|u' => \$unscaffolded, 'version|v' => \$dispversion, 'help|h|?+' => \$help, man => \$man) or pod2usage(2);
...
if ($unscaffolded != 0) {
   for my $key (keys %ctg_lengths) {
      print $agp join("\t", $key, "1", $ctg_lengths{$key}, "1", "W", $key, "1", $ctg_lengths{$key}, "+"), "\n" unless exists($scaffolded_contigs{$key});
   }
}
close($agp);
exit 0;
